...
class Mapper1(Mapper):
    ...
    def cpu_read(self, addr):
        ...
        if addr < 0xC000:
            ...
            if prg_mode == 0 or prg_mode == 1:
                bank = bank & 0xFE
                mapped_addr = (bank + 1) * 0x4000 + (addr - 0xC000)
            elif prg_mode == 2:
    ...
    def ppu_read(self, addr):
        ...
        if self.use_chr_ram:
            ...
            return self.chr_rom[mapped_addr % len(self.chr_rom)]
...
class Bus:
    ...
    def load_rom(self, rom: NESRom):
        ...
        if rom.mapper == 0:
            ...
            self.mapper = Mapper0(prg_banks, chr_banks)  # Fallback
    ...
    def set_controller(self, controller, buttons):
        """Set controller state (A,B,Select,Start,Up,Down,Left,Right)"""
        if controller == 1:

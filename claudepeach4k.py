...
# Sky - using a simple colored entity instead of Sky()
...
moat_inner = Entity(model='circle', scale=45, color=color.rgb(34, 139, 34), y=0.02, segments=64, collider='box')
...
camera.fov = 90
camera.position = Vec3(10, 10, -35)  # Start further back
...
def update():
    ...
    # Camera follows player (Mario 64-style) - smoother following
    if camera_initialized:
        target_pos = player.position + Vec3(10, 10, -20)
        camera.position = Vec3(
            camera.position.x + (target_pos.x - camera.position.x) * 2 * time.dt,
            camera.position.y + (target_pos.y - camera.position.y) * 2 * time.dt,
            camera.position.z + (target_pos.z - camera.position.z) * 2 * time.dt
        )

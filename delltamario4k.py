...
# - All art is generated with code; no images or sounds required. You can add your own later.
...
WALK_SPEED          = 140.0
RUN_SPEED           = 220.0
ACCEL_FACTOR        = 0.20    # smoother acceleration toward target speed
JUMP_SPEED          = 560.0
COYOTE_TIME         = 0.10    # seconds after leaving an edge where a jump still works
JUMP_BUFFER         = 0.12    # seconds to queue a jump before landing
TERMINAL_VY         = 2000.0
MAX_LEVELS          = 32
LEVEL_HEIGHT_TILES  = SCREEN_H // TILE  # 30 at 480p with TILE=16
# Width varies by level, but generally 160–240 tiles depending on generator choices.
...
class Level:
    def __init__(self, index):
        ...
        base_w = 170 + (index % 4) * 12 + (index // 4) * 6
        jitter = self.rng.randint(-8, 10)

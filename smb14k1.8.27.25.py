...
pygame.display.set_caption("Enhanced Super Mario RPG")
...
class Player(Character):
    ...
    def draw(self, screen):
        pygame.draw.rect(screen, self.color, (self.x, self.y + 15, self.width, 25))
        pygame.draw.circle(screen, self.color, (self.x + 15, self.y + 10), 10)
        pygame.draw.arc(screen, RED, (self.x + 5, self.y, 20, 20), 0, math.pi, 5)
        pygame.draw.rect(screen, BLACK, (self.x + 8, self.y + 12, 14, 2))

class Party:
    def __init__(self, player):
        self.members = [player]
        self.active = [player]  # Up to 3 active in battle
...
# Expanded maps based on SMRPG locations
maps = {
